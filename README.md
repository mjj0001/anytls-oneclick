...
- ⌨️ **快捷命令设置菜单**：主菜单 `16)` 可随时更换/删除/修复自定义快捷命令

## 🆕 更新 (v3.4.12)

- ⌨️ **自定义快捷指令**：首次运行提示设置一个字母/数字命令，之后可一键进入主菜单
...
> 
> **自定义快捷指令（新增）**：首次运行脚本会提示你设置一个“字母/数字命令”（如 `a` / `k2` / `go`），以后输入它即可直接打开主菜单。
...
- `16) 快捷命令设置`：设置/更换/删除你自定义的字母数字快捷命令
